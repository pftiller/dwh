      ...
      FROM 
        (
          ...
          FROM 
            ...
            OUTER JOIN stage.signal_match_table mtch ON mtch.suid = combined.suid 
          WHERE 
            mtch.suid IN (
              SELECT 
                suid 
...
SELECT 
  ...
  CASE WHEN c.online_orders_in_period IS NULL THEN 0 WHEN c.online_orders_in_period IS NOT NULL THEN c.online_orders_in_period END AS online_orders_in_period, 
  CASE WHEN d.total_impressions IS NULL THEN 0 WHEN d.total_impressions IS NOT NULL THEN d.total_impressions END AS total_impressions, 
  CASE WHEN c.online_order_amount_in_period IS NULL THEN 0.00 WHEN c.online_order_amount_in_period IS NOT NULL THEN c.online_order_amount_in_period END AS online_order_amount_in_period, 
  d.first_ad_impression AS first_ad_impression, 
  d.last_ad_impression AS last_ad_impression, 
  CASE WHEN e.total_page_views IS NULL THEN 0 :: text WHEN e.total_page_views IS NOT NULL THEN e.total_page_views END AS total_page_views, 
...
FROM 
  t1 FULL 
  OUTER JOIN stage.signal_slumberland_crm crm ON t1.value_txt = crm.cust_id FULL 
  OUTER JOIN (
    SELECT 
      DISTINCT crm.cust_id AS cust_id, 
      COUNT(DISTINCT crm.order_id) AS lifetime_transactions, 
      SUM(
        crm.line_amt :: NUMERIC (8, 2)
      ) AS lifetime_order_amt 
    FROM 
      stage.signal_slumberland_crm crm 
    GROUP BY 
      crm.cust_id
  ) AS a ON t1.value_txt = a.cust_id FULL 
  OUTER JOIN (
    SELECT 
      ...
      SUM(
        crm.line_amt :: NUMERIC (8, 2)
      ) AS order_amount_in_period 
    FROM 
      stage.signal_slumberland_crm crm 
    WHERE 
      crm.order_dt BETWEEN <Parameters.start_date>  
      AND <Parameters.end_date> 
    GROUP BY 
      crm.cust_id
  ) AS b ON t1.value_txt = b.cust_id FULL 
  OUTER JOIN (
    SELECT 
      DISTINCT crm.cust_id, 
      crm.order_id, 
      ...
      SUM(
        cart_total_amt :: NUMERIC (8, 2)
      ) AS online_order_amount_in_period 
    FROM 
      stage.signal_slumberland_crm crm 
      JOIN stage.signal_page_view pgvw ON pgvw.order_id = crm.sfcc_order_id 
    WHERE 
      regexp_count(cart_total_amt, '\\.') <= 1 
      AND signal_ts BETWEEN (
        <Parameters.end_date> :: date - '120 days' :: interval
      ) 
      AND <Parameters.end_date> 
    GROUP BY 
      crm.cust_id, 
      crm.order_id, 
      crm.sfcc_order_id
  ) AS c ON t1.value_txt = c.cust_id 
  ...
  OUTER JOIN (
    ...
    WHERE 
      ...
      AND pgvw.signal_ts BETWEEN (
        <Parameters.end_date> :: date - '120 days' :: interval
      ) 
      AND <Parameters.end_date> 
    GROUP BY 
      pgvw.signal_id
  ) AS e ON t1.suid = e.suid FULL 
  OUTER JOIN (
    SELECT 
    ...
    WHERE 
      ttd.signal_ts BETWEEN (
        <Parameters.end_date> :: date - '120 days' :: interval
      ) 
      AND <Parameters.end_date> 
    GROUP BY 
      ttd.signal_id
  ) AS d ON t1.suid = d.signal_id FULL 
  OUTER JOIN (
    SELECT 
